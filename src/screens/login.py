...
from core.data_manager import sign_in
...
class LoginScreen:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if mouse_in_button(login_button, mouse_pos):
                        ...
                        if self.username is None or self.password is None:
                            show_error_popup("Please provide password and username to log-in.")
                        
                        else:
                            # Trying to sign-in
                            success, data = sign_in(self.username, self.password)
                            
                            # If failed, fetch an error message
                            if success is False:
                                show_error_popup("Username or password incorrect.")
                            
                            # If success, move user to another screen
                            else:
                                self.screen_manager.username = self.username
                                self.screen_manager.set_screen("menu")

                    # Move-to-sign-up button click check
                    elif mouse_in_button(move_to_signup_button, mouse_pos):
                        self.screen_manager.set_screen("register")
    ...
    def draw(self, screen):
        ...
        # Main background
        background = load_and_scale('assets/login-bg.jpg', WINDOW_WIDTH, WINDOW_HEIGHT)
